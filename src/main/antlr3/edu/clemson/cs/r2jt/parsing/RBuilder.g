...
@members{
    ...
    boolean performanceEModule = false;
    boolean performanceCModule = false;
...
@init{
    ...
    :   ^(  FACILITY ps=ident
            cName=ident (cPars=module_argument_section)?
            (eItem=facility_enhancement { eItems.add($eItem.item); })*
            REALIZED bName=ident (WITH_PROFILE pName=ident)? (bPars=module_argument_section)?
            (ebItem=facility_body_enhancement { ebItems.add($ebItem.item); })*
        )
...
elapsed_time_clause returns [Exp exp = null]
    :   ^(ELAPSED_TIME exp1=adding_expression) { $exp = $exp1.exp; }
    ;
